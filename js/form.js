...
const normalizeTags = (tagStr) => {
  tagStr.trim().split(' ').filter((item) => Boolean(item.length));
};
...
pristine.addValidator(hashtagField, checkTagUnique, ErrorMessage.NON_UNIQUE_HASHTAG, 1 , true);

pristine.addValidator(hashtagField, hasValidTags, ErrorMessage.INVALID_HASHTAG_PATTERN, 2 , true);

pristine.addValidator(hashtagField, hasValidCount, ErrorMessage.INVALID_HASHTAG_COUNT, 3 , true);
